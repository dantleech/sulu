...
class SubscriberPass implements CompilerPassInterface
{
    public function process(ContainerBuilder $container)
    {
        ...
        foreach ($container->findTaggedServiceIds('sulu_document_manager.event_subscriber') as $subscriberId => $attributes) {
            ...
            // if document mangager attribute is set, add the subscriber ONLY to the event dispatcher
            // of the specified document manager.
            $targetDispatcherIds = [];
            ...
            foreach ($targetDispatcherIds as $dispatcherId) {
                ...
                $dispatcher->addMethodCall('addSubscriberService', array($subscriberId, $class));
            }
        }
    }
}
